...
# Migration limit (set to None for all)
MIGRATION_LIMIT = 10   # change to None for unlimited

# =========================================================================
# FIELD SELECTION
# =========================================================================
# Set to a list of Freshservice field keys you want to migrate.
# Example: ["serial_number"] -> only serial number
#          [] or None -> migrate all defined fields
MIGRATE_ONLY_FIELDS = ["serial_number"]   # <-- CHANGE THIS to test serial

# Hardcode Jira attribute ID for serial number if needed
JIRA_SERIAL_ATTR_ID_OVERRIDE = None

# =========================================================================
# CACHES & MAPPING – now using root keys where appropriate
# =========================================================================
DEPARTMENT_CACHE = {}
LOCATION_CACHE = {}
USER_CACHE = {}
...
JIRA_ASSET_MAPPING = {
    ...
    "assigned_on": {"name": "Assigned On", "type": 4},           # root
    ...
    "warranty_expiry_date": {"name": "Warranty Expiry Date", "type": 4},  # ROOT
    ...
    "serial_number": {"name": "Serial Number", "type": 0},                # ROOT
    "state_26000466695": {"name": "State", "type": 0},
    "provider_type_26000466695": {"name": "Provider", "type": 0}
}

# =========================================================================
# HELPER FUNCTIONS (unchanged – but note resolve functions still use root IDs)
# =========================================================================
...
def check_jira_duplicate_by_identifiers(serial_number, asset_tag, jira_field_map):
    ...
    # Use override or lookup by name
    serial_attr_id = JIRA_SERIAL_ATTR_ID_OVERRIDE or jira_field_map.get("serial number")
...
def fetch_freshservice_assets(page=1):
    ...
    params = {"asset_type_id": FS_LAPTOP_TYPE_ID, "per_page": 50, "page": page}
...
def migrate_assets():
    try:
        ...
        if MIGRATION_LIMIT:
            print(f"[*] LIMIT ACTIVE: Will stop after {MIGRATION_LIMIT} successful migrations.")
        else:
            print(f"[*] NO LIMIT: Will migrate all available laptop assets.")

        if MIGRATE_ONLY_FIELDS:
            print(f"[*] FIELD FILTER ACTIVE: Only migrating these keys: {MIGRATE_ONLY_FIELDS}")
        else:
            print(f"[*] FIELD FILTER: Migrating all defined fields.")

        if JIRA_SERIAL_ATTR_ID_OVERRIDE:
            print(f"[*] SERIAL OVERRIDE: Using hardcoded Jira attribute ID {JIRA_SERIAL_ATTR_ID_OVERRIDE} for serial number.")
        ...
        while page <= max_pages:
            ...
            for asset in fs_assets:
                if MIGRATION_LIMIT and successfully_migrated >= MIGRATION_LIMIT:
                    break
                ...
                # Merge: root takes precedence, then type_fields, then resolved
                fs_pool = {**asset, **type_fields, **resolved_references}

                # ---- Get serial from root (now directly) ----
                serial_num = fs_pool.get("serial_number")  # root field
                asset_tag = fs_pool.get("asset_tag")

                if check_jira_duplicate_by_identifiers(serial_num, asset_tag, jira_field_map):
                    print(f"   [SKIP] Asset '{asset_name}' (Serial: {serial_num}) already exists in Jira.")
                    continue
                ...
                # ---- MAPPED FIELDS (with filtering) ----
                for fs_key, target_info in JIRA_ASSET_MAPPING.items():
                    if MIGRATE_ONLY_FIELDS and fs_key not in MIGRATE_ONLY_FIELDS:
                        continue
                    ...
                    if fs_key in fs_pool:
                        ...
                        if fs_key == "serial_number" and JIRA_SERIAL_ATTR_ID_OVERRIDE:
                            attr_id = JIRA_SERIAL_ATTR_ID_OVERRIDE
                        else:
                            attr_id = jira_field_map.get(target_attr_lowercase)
            ...
            if MIGRATION_LIMIT and successfully_migrated >= MIGRATION_LIMIT:
                print(f"\n[LIMIT REACHED] Stopped after {successfully_migrated} successful migrations.")
                break

            page += 1

        print(f"\n[COMPLETE] Migration finished. Successfully migrated {successfully_migrated} assets.")
